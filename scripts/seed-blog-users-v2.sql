...
INSERT INTO blog_users (email, name, password_hash, role, is_active) VALUES 
(
  'info@gcmasesores.io',
  'GCM Asesores Admin',
...
DECLARE
    admin_user_id UUID;
BEGIN
    SELECT id INTO admin_user_id FROM blog_users WHERE email = 'info@gcmasesores.io';
...
SELECT title, slug, published, featured, created_at FROM posts ORDER BY created_at;
